from app.ai.embeddings import selecionar_candidatos_similares, serializar_embedding


def test_selecionar_candidatos_similares_devolve_mais_proximo_primeiro():
    embeddings_canonicos = {
        1: serializar_embedding([1.0, 0.0]),
        2: serializar_embedding([0.0, 1.0]),
        3: serializar_embedding([0.9, 0.1]),
    }
...
def test_selecionar_candidatos_similares_une_top_k_de_varias_descricoes_sem_duplicar():
    embeddings_canonicos = {1: serializar_embedding([1.0, 0.0]), 2: serializar_embedding([0.0, 1.0])}
...
def test_selecionar_candidatos_similares_respeita_max_total():
    embeddings_canonicos = {i: serializar_embedding([float(i), 1.0]) for i in range(5)}
...
def test_selecionar_candidatos_similares_ignora_descricao_sem_embedding():
    embeddings_canonicos = {1: serializar_embedding([1.0, 0.0])}
